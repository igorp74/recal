    ...
    if let Ok(file) = fs::File::open(filename) {
        ...
        for (_line_num, line) in reader.lines().enumerate() {
            if let Ok(line) = line {
                ...
                // Check for Fixed Date Rule (which should recur annually and count years)
                if let Some(date) = parse_fixed_date_rule(rule_part) {
                    // If we successfully parse a fixed date, this is an anniversary rule (DD-MM-YYYY or MM/DD/YYYY)
                    base_date = Some(date);
